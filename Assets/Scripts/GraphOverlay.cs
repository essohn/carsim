...
public class GraphOverlay : MonoBehaviour
{
	...
	public class CarData {
		...
		public List<float> velocityData;
		public List<float> rollData;
    ...
    public class WheelConfig
    {
		...
		public List<float> forceData = new List<float>();
	...
	public Color32 rollColor = Color.magenta;
	...
	void Start()
	{
		...
		simManager = GetComponentInParent<SimManager> ();
	...
	void FixedUpdate()
	{
		if (simManager.simState != SimManager.SimState.END) {
			...
			foreach (var wheelConfig in wheelConfigs)
			{
				...
				if (!wheelConfig.collider.GetGroundHit(out hit))
				{
					// No hit!
//					continue;
					wheelConfig.longData.Add(0);
					wheelConfig.latData.Add(0);
					wheelConfig.forceData.Add (0);
				} else 
				...
				if (Mathf.Abs(timeTravel) < k_MaxRecordTimeTravel)// TODO: solve this mystery
				{
					...
					wheelConfig.forceData.Add (hit.force);
			...
			if (Mathf.Abs(timeTravel) < k_MaxRecordTimeTravel)// TODO: solve this mystery
			{
				carData.timeData.Add (simManager.simTimer);

				carData.rollData.Add (vehicleBody.rotation.eulerAngles.z);

				carData.velocityData.Add (vehicleBody.velocity.magnitude * 3600f / 1000f);
			}
...
//			for (int i = cursor; i < wheelConfig.latData.Count - 1 - stepsBack; ++i)
//			{
//				DrawLine( PlotSpace(cursor, i, carData.rollData[i] / 10.0f ), PlotSpace(cursor, i + 1, carData.rollData[i+1] / 10.0f), rollColor );
//			}
